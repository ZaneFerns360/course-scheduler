...
const BlogCard: React.FC<Post> = ({
  ...
  return (
    ...
    >
      <Link href={`/home/${id}`}>
        <div className="h-full shadow-lg rounded-lg overflow-hidden transition-shadow duration-300 hover:shadow-xl">
          <img
            src={imageUrl}
            alt={title}
  ...
  return (
    <div className="bg-gray-100 min-h-screen px-4 py-12">
      <div className=" mx-auto">
        <h2 className="text-4xl md:mt-7 font-serif mt-14 font-bold text-gray-800 mb-6 text-center">
          Discover Our Latest Articles
        </h2>
        <div className="mb-12">
          <div className="flex justify-start md:justify-center gap-4 md:overflow-auto overflow-visible overflow-x-scroll">
            {courses.map((course) => (
              <motion.button
                key={course.id}
                className={`px-5 py-2 font-semibold shadow-lg my-4 rounded-2xl transition-colors duration-300 ${
        ...
        <motion.div
          key={activeCourse}
          className="flex flex-wrap -m-4 container mx-auto"
